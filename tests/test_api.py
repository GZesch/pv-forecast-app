...
from frontend.session_identity import stable_session_id_from_code
...
def test_debug_open_meteo_endpoint(tmp_path, monkeypatch) -> None:
...
def test_installations_remain_visible_for_same_stable_user_code(
    ...
    owner_headers = {
        "X-Session-ID": str(stable_session_id_from_code("Mein Projekt"))
    }
    same_code_headers = {
        "X-Session-ID": str(stable_session_id_from_code("  mein projekt  "))
    }

    with TestClient(app, headers=owner_headers) as owner:
        created = owner.post("/installations", json=payload).json()

    with TestClient(app, headers=same_code_headers) as returning_user:
        response = returning_user.get("/installations")
...
def test_pv_forecast_endpoint(tmp_path, monkeypatch) -> None:
    ...
    expected_timestamp = datetime.fromisoformat("2026-06-19T12:00:00+00:00")
    assert datetime.fromisoformat(
        history[0]["forecast_start"].replace("Z", "+00:00")
    ) == expected_timestamp
    assert datetime.fromisoformat(
        history[0]["forecast_end"].replace("Z", "+00:00")
    ) == expected_timestamp
    ...
    assert datetime.fromisoformat(
        history[0]["peak_timestamp"].replace("Z", "+00:00")
    ) == expected_timestamp
    ...
    with duckdb.connect(str(database_path), read_only=True) as connection:
        ...
        stored_run = connection.execute(
            """
            SELECT target_type, target_id, installation_id, hourly_json, source
            FROM forecast_runs
            """
        ).fetchone()
        assert stored_run[0] == "installation"
        assert str(stored_run[1]) == installation["id"]
        assert str(stored_run[2]) == installation["id"]
        assert "predicted_power_kw" in stored_run[3]
        assert stored_run[4] in {"fresh", "cached"}
...
def test_installation_forecast_uses_stored_forecast_on_rate_limit(
    tmp_path, monkeypatch
) -> None:
    database_path = tmp_path / "test.duckdb"
    monkeypatch.setenv("DATABASE_PATH", str(database_path))
    ...
    async def fake_geocode(_: str) -> Coordinates:
        return Coordinates(latitude=59.32512, longitude=18.07109)
    ...
    async def rate_limited_weather(**_) -> list[WeatherForecastRow]:
        raise WeatherServiceRateLimitError("Daily API request limit exceeded")

    def fake_calculate(**_) -> list[PVForecastRow]:
        return [
            PVForecastRow(
                timestamp="2026-06-21T10:00:00Z",
                predicted_power_kw=4.25,
            )
        ]
    ...
    with TestClient(app) as client:
        ...
        fresh = client.get(f"/installations/{installation['id']}/pv-forecast")
        monkeypatch.setattr(
            "backend.main.open_meteo_service.get_hourly_forecast",
            rate_limited_weather,
        )
        fallback = client.get(f"/installations/{installation['id']}/pv-forecast")

    assert fresh.status_code == 200
    assert fallback.status_code == 200
    assert fallback.json()["hourly"] == fresh.json()["hourly"]
    assert fallback.json()["daily"] == fresh.json()["daily"]
    assert "Tageslimit erreicht" in fallback.json()["warning"]
    with duckdb.connect(str(database_path), read_only=True) as connection:
...
def test_plant_forecast_sums_component_installations(tmp_path, monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        fallback_response = client.get(f"/plants/{plant['id']}/pv-forecast")
    ...
    assert fallback_response.json()["hourly"] == payload["hourly"]
    assert fallback_response.json()["components"] == []
    assert "Tageslimit erreicht" in fallback_response.json()["warning"]
    with duckdb.connect(str(tmp_path / "test.duckdb"), read_only=True) as connection:
        ...
        assert stored[0] == "plant"
        assert str(stored[1]) == plant["id"]
        assert stored[2] is None
        assert "predicted_power_kw" in stored[3]
        assert stored[4] in {"fresh", "cached"}
